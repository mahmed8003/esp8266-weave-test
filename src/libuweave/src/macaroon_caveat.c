...
static bool is_valid_caveat_type_(UwMacaroonCaveatType type) {
  switch (type) {
    ...
    case kUwMacaroonCaveatTypeTTL30Day:
...
bool uw_macaroon_caveat_create_ttl_30_day_(uint8_t* buffer,
                                           size_t buffer_size,
                                           UwMacaroonCaveat* new_caveat) {
  return create_caveat_no_value_(kUwMacaroonCaveatTypeTTL30Day, buffer,
                                 buffer_size, new_caveat);
}
...
bool uw_macaroon_caveat_validate_(const UwMacaroonCaveat* caveat,
  ...
  switch (caveat_type) {
    ...
    case kUwMacaroonCaveatTypeTTL30Day:
      ...
      return update_and_check_expiration_time(
          context->current_time, state->issued_time + 30 * 24 * 60 * 60,
          result);
